...
export default function ProceduresPage() {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50" data-testid="procedures-page">
      <Sidebar />
      <main className="flex-1">
        <TopBar title="Procedimientos de Seguridad" subtitle="Gestión documental con análisis de IA">
          <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
            <DialogTrigger asChild>
              <Button className="bg-blue-500 hover:bg-blue-600 rounded-lg gap-2" data-testid="add-procedure-btn">
            ...
            <DialogContent className="max-w-lg">
                  ...
                  >
                    ...
                    {selectedFile ? (
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-slate-600">Arrastra o haz clic para subir</p>
              ...
              <DialogFooter>
                ...
                <Button
                  ...
                  className="bg-blue-500 hover:bg-blue-600 rounded-lg gap-2"
        ...
        <div className="p-8 space-y-6">
          ...
          <Card className="border-blue-200 bg-blue-50">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                <h3 className="font-semibold text-slate-900">Integración con Scan 360°</h3>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {procedures.map((proc) => (
                <Card key={proc.id} className="border-slate-200 hover:shadow-md transition-shadow">
                  ...
                  <CardContent>
                    {proc.ai_analysis && (
                      <p className="text-sm text-slate-600 mb-4 line-clamp-2">{proc.ai_analysis}</p>
                    )}
                    ...
                    <div className="space-y-2 mb-4">
                      {proc.risks_identified?.length > 0 && (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="text-slate-600">{proc.risks_identified.length} riesgos identificados</span>
                        </div>
                      )}
                      {proc.controls_required?.length > 0 && (
                        <div className="flex items-center gap-2 text-sm">
                          <Shield className="w-4 h-4 text-blue-500" />
                          <span className="text-slate-600">{proc.controls_required.length} controles requeridos</span>
                        </div>
                      )}
                      {proc.epp_required?.length > 0 && (
                        <div className="flex items-center gap-2 text-sm">
                          <HardHat className="w-4 h-4 text-green-500" />
                          <span className="text-slate-600">{proc.epp_required.length} EPP requeridos</span>
                        </div>
                      )}
                    </div>
              ...
              {selectedProcedure && (
                <ScrollArea className="max-h-[60vh]">
                  <div className="space-y-6 py-4">
                    ...
                    {/* Content Preview */}
                    <div>
                      <h4 className="font-medium text-slate-900 mb-2">Contenido del Documento</h4>
                      <div className="p-4 bg-slate-50 rounded-lg text-sm text-slate-600 font-mono whitespace-pre-wrap max-h-48 overflow-y-auto">
