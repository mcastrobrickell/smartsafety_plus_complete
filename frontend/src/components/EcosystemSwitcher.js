...
const APP_COLORS = {
  smartsafety: 'bg-gradient-tech',
  smartplan: 'bg-purple-500',
  smartforms: 'bg-success-green'
};
  ...
  if (loading || !currentApp) {
    return (
      <div className="flex items-center gap-3 px-4 py-3">
        <div className="w-10 h-10 bg-gradient-tech rounded-xl animate-pulse"></div>
        <div className="h-4 bg-dark-elevated rounded w-24 animate-pulse"></div>
      </div>
    );
  }
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button 
          className="flex items-center gap-3 px-4 py-3 w-full hover:bg-cyan-accent/5 transition-colors rounded-lg"
          data-testid="ecosystem-switcher"
        >
          <div className={`w-10 h-10 ${APP_COLORS[currentApp.code] || 'bg-gradient-tech'} rounded-xl flex items-center justify-center flex-shrink-0 shadow-glow-cyan`}>
            <CurrentIcon className="w-6 h-6 text-dark-bg" />
          </div>
          <div className="flex-1 text-left min-w-0">
            <div className="font-bold text-sm truncate font-display">
              <span className="text-cyan-accent">Smart</span>
              <span className="logo-gradient">{currentApp.name.replace('Smart', '')}</span>
            </div>
      ...
      <DropdownMenuContent align="start" className="w-64 bg-dark-surface border-cyan-accent/20">
        <DropdownMenuLabel className="text-xs text-cyan-accent font-normal">
          Ecosistema Smart+
        </DropdownMenuLabel>
        <DropdownMenuSeparator className="bg-cyan-accent/10" />
        ...
        {apps.map((app) => {
          ...
          return (
            <DropdownMenuItem
              key={app.id}
              className={`flex items-center gap-3 p-3 cursor-pointer hover:bg-cyan-accent/10 ${
                isDisabled && !app.is_current ? 'opacity-50' : ''
              }`}
              onClick={() => handleAppSwitch(app)}
              disabled={isDisabled && !app.is_current}
            >
              <div className={`w-9 h-9 ${APP_COLORS[app.code] || 'bg-slate-600'} rounded-lg flex items-center justify-center`}>
